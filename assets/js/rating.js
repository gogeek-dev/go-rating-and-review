...
function Star1() {
    var valueData = $('#star1').val() ;
    {{/* alert(valueData) */}}
        if(valueData == 1){
          $('.comment').text('Very bad');
        } 
         return false;
 }
 function Star2() {  
    var valueData = $('#star2').val() ;
    {{/* alert(valueData) */}}
        if(valueData == 2){
          $('.comment').text('Bad');
        } 
 ...
 $('#btn_Rating').click(function() {  
      ...
      var name = $("#Review").val();
      if(name == '') {
        $("#Review").after('<span style="color: rgb(228, 60, 18); font-size: 10pt" class="error1">*Please Enter your review.</span>');
        hasError = true;
      } 
